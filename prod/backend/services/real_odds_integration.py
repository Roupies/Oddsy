...
# Import modules avec graceful fallback
try:
    from monitoring.real_odds_validator_v53 import RealOddsValidatorV53
except ImportError:
    # Fallback pour démo - validation désactivée si module absent
    RealOddsValidatorV53 = None
...
class RealOddsIntegrationService:
    ...
    def __init__(self, odds_dir: str = "data/odds", config_path: str = "config/odds_sources.yaml"):
        ...
        try:
            self.validator = RealOddsValidatorV53(str(self.config_path)) if RealOddsValidatorV53 else None
    ...
    def _detect_next_gameweek(self) -> int:
        ...
        calendar_path = Path(__file__).parent.parent.parent / "data/EPL_25_26_Full_Calendar.csv"
    ...
    def _fetch_odds_from_source(self, source: str, home_team: str, away_team: str, match_date: str) -> Dict:
        ...
        # ONLY real API calls - no simulation/fallback
        return self._fetch_real_odds_api(source, home_team, away_team, match_date)
    
    def _fetch_real_odds_api(self, source: str, home_team: str, away_team: str, match_date: str) -> Dict:
        """Fetch real odds from the-odds-api.com"""
        try:
            import requests
            
            api_key = os.getenv('THE_ODDS_API_KEY')
            if not api_key or api_key == 'your_odds_api_key_here':
                self.logger.warning("⚠️ THE_ODDS_API_KEY not configured, fallback to simulation")
                return None
            
            # Map team names to the-odds-api format (they use different naming)
            team_mapping = {
                'Arsenal': 'Arsenal',
                'Burnley': 'Burnley',
            ...
            if response.status_code != 200:
                self.logger.error(f"❌ Odds API error {response.status_code}: {response.text}")
                return None
            ...
            for game in data:
                if (home_mapped in game.get('home_team', '') and away_mapped in game.get('away_team', '')) or \
                   (away_mapped in game.get('home_team', '') and home_mapped in game.get('away_team', '')):
                    
                    # Get bookmaker odds (prefer bet365, pinnacle)
                    for bookmaker in game.get('bookmakers', []):
                        if bookmaker['key'] in ['bet365', 'pinnacle', 'williamhill', 'betfair']:
                            for market in bookmaker.get('markets', []):
                                if market['key'] == 'h2h':
                                    outcomes = {outcome['name']: outcome['price'] for outcome in market['outcomes']}
                                    
                                    # Map outcomes to our format
                                    home_odds = outcomes.get(home_mapped, outcomes.get(game['home_team']))
                                    away_odds = outcomes.get(away_mapped, outcomes.get(game['away_team']))
            ...
            self.logger.warning(f"⚠️ No odds found for {home_team} vs {away_team} in API response")
            return None
            
        except Exception as e:
            self.logger.error(f"❌ Real odds API error: {e}")
            return None

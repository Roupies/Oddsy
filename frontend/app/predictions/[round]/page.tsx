...
export default function MatchdayPage({ params }: MatchdayPageProps) {
    ...
    queryFn: async () => {
      try {
        ...
        if (error.message?.includes('422') && round >= 8) {
          console.log(`⚠️ J${round}: Fallback vers mode multi-GW après échec strict`);
  ...
  console.log('PredictionsData received:', predictionsData)
  ...
  const isCompatMode = predictionsData.meta?.compat_mode_warning || false
  const modeEffective = predictionsData.data?.meta?.mode_effective || 'strict_epl'
  ...
  return (
    <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-xs text-amber-400 mt-1">
                Pipeline v2.0 détecté avec {totalMatches} matchs sur {predictionsData.data?.meta?.distinct_dates?.length || 0} dates. 
      ...
      <div className="mb-8">
        ...
        <p className="text-neutral-400 mb-6">
          Pipeline Durci v1.0 predictions for Matchday {round}
        </p>
        
        <div className="flex flex-wrap gap-4 text-sm">
          <span className="badge badge-success">✅ Enhanced Baseline v2.4</span>
      ...
      {/* Séparation Résultats vs Prédictions (BACKUP) */}
      {false && predictionsData.data?.meta?.temporal_breakdown && (
        <div className="space-y-8">
          {/* Résultats joués */}
          {predictionsData.data.results_played && predictionsData.data.results_played.length > 0 && (
            <div>
              <h2 className="text-2xl font-bold text-white mb-4 flex items-center">
                <span className="text-green-600 mr-2">✅</span>
                Résultats J{round} ({predictionsData.data.results_played.length} matchs joués)
              </h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {predictionsData.data.results_played.map((match) => (
                  <MatchCardModern 
                    key={match.id} 
                    match={match}
                  />
          ...
          {/* Prédictions à venir */}
          {predictionsData.data.predictions_upcoming && predictionsData.data.predictions_upcoming.length > 0 && (
            <div>
              <h2 className="text-2xl font-bold text-white mb-4 flex items-center">
                <span className="text-blue-600 mr-2">🔮</span>
                Prédictions J{round} ({predictionsData.data.predictions_upcoming.length} matchs à venir)
              </h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {predictionsData.data.predictions_upcoming.map((match) => (
                  <MatchCardModern 
                    key={match.id} 
                    match={match}
                  />
          ...
          {/* Fallback: tous les matchs si pas de séparation temporelle */}
          {(!predictionsData.data.results_played || predictionsData.data.results_played.length === 0) && 
           (!predictionsData.data.predictions_upcoming || predictionsData.data.predictions_upcoming.length === 0) && (
            <div>
              <h2 className="text-2xl font-bold text-white mb-4">
                Tous les matchs J{round}
              </h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {matches.map((match) => (
                  <MatchCardModern 
      ...
      <div className="mt-12 text-center">
        <div className="card bg-neutral-900/60 border border-neutral-700 max-w-2xl mx-auto backdrop-blur-sm">
          ...
          <div className="text-sm text-neutral-400 space-y-2">
            <p>
              Generated by Pipeline Durci v1.0 using Enhanced Baseline v2.4 with strict temporal validation.
            </p>
            <p>

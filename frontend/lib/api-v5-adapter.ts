...
export async function fetchGameweekPredictions(
  ...
  try {
    const response = await fetch(`${baseURL}/api/v5/gameweeks/${gameweek}/predictions`)
    ...
    if (rawData.predictions) {
      ...
      const matches = Object.entries(rawData.predictions).map(([matchKey, predData]: [string, any], index) => {
        ...
        return {
          ...
          date: predData.match_info?.date || '2025-10-20',
          round: gameweek,
      ...
      const result = {
        ...
        data: {
          ...
          ensemble_system: {
            system_name: 'Enhanced Baseline',
            version: rawData.metadata?.model_version || 'v2.4',
            models: {
              enhanced_baseline: {
                name: 'Enhanced Baseline v2.4',
                accuracy: 0.535,
